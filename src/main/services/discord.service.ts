...
class DiscordRPCManager {
  ...
  async disable() {
    if (this._createClientAbortController) {
      ...
      this.logger.debug(
        "aborting client creation",
        this._createClientAbortController.signal.aborted,
      );
    }
    ...
    return new Promise<void>((resolve, reject) => {
      if (this.client && this._isConnected)
        (initial ? Promise.resolve() : this.setActivity(this.presence, false)).then(() => {
          resolve();
          return (this._updateHandle = setTimeout(
...
export default class DiscordProvider extends BaseProvider implements AfterInit {
  ...
  get presenceTitle() {
	const raw = this.settingsInstance.get("discord.title", "ytm");
	switch (raw) {
		case "ytm":
    ...
    if (this.trackService.trackData) {
      ...
      if (track) {
        await this.rpcManager.setActivity(
          discordEmbedFromTrack(track, this.presenceTitle, playing, trackState?.progress ?? 0),
        );
      }
    }
  }
  ...
  async updateTrackProgress(isPlaying: boolean, mediaProgress: number = 0) {
    if (this.trackService.trackData && this.isConnected) {
      await this.rpcManager.setActivity(
        discordEmbedFromTrack(this.trackService.trackData, this.presenceTitle, isPlaying, mediaProgress),
      );
    }
  }
  ...
  private async __onTrackInfo(track: TrackData) {
    ...
    await this.rpcManager.setActivity(discordEmbedFromTrack(track, this.presenceTitle));
  }
}
